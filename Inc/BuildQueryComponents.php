...
class buildQueryComponents
{
    ...
    function build($queryString): bool|array
    {
        ...
        foreach($commas as $comma_queryString) {
            ...
            preg_match('/([-+])?([0-9])?(n)?([-+][0-9]+)?/', $comma_queryString, $preg_match_matches_pseudo_classes_values);
            $forbidden_pseudo_classes_values = [
                "",
                "+",
                "-",
            ];

            if(isset($preg_match_matches_pseudo_classes_values[0]) && !in_array($preg_match_matches_pseudo_classes_values[0], $forbidden_pseudo_classes_values, true)) {
                $queryStringRebuilded .= $comma_queryString;
                $is_nth_string = true;
            }

            // -- get element name
            if($is_nth_string === false) {
                ...
                if (isset($preg_match_matches_pseudo_classes[0])) {
                    ...
                    foreach ($pseudoClassNames as $pseudoClassName) {
                        $getBetween = new getBetween();
                        $pseudoClassNameArray = $getBetween->getArray($preg_match_matches_pseudo_classes, "(", ")");
